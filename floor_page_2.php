...
if (!$floor || !in_array($floor, [1, 2, 3, 4])) {
        ...
        if ($name != $_SESSION['f_name']) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($studentData['room_id']) {
                    ...
                    $incrementStmt = $conn->prepare("UPDATE room_alloc_boys SET no_of_mem = no_of_mem + 1 WHERE room_id = ?");
                    $incrementStmt->bind_param("s", $roomId);
                    $incrementStmt->execute();

                    // Check if no_of_mem exceeds the limit (3)
                    $checkRoomStmt = $conn->prepare("SELECT no_of_mem FROM room_alloc_boys WHERE room_id = ?");
...
<head>
    ...
    <title>basaveshwara Hostel - Floor <?php echo $floor; ?></title>
    <style>
        body {
...
<body>
    <header>
        <h1>BASAVESHWARA HOSTEL - FLOOR<?php echo $floor; ?></h1>
    </header>
    <div class="background-image">
        ...
        <img src="https://tse1.mm.bing.net/th?id=OIP.0gveq3iUuo3zwFEGY6N8YgHaEo&pid=Api&P=0&h=180" alt="Background Image" style="width: 100%; height: 100%; object-fit: cover;">
    </div>
